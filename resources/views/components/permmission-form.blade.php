    ...
    <main>
      <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10 bg-gray-50">
        <div class="border p-10 bg-white rounded-md">
          <h1 class="font-bold my-3 text-3xl">Permission {{$type === 'create' ? 'Create':'Edit'}}</h1>
          <form class="space-y-4 md:space-y-6" method="post" action="{{$type === 'create' ?  route('permissions.store'): route('permissions.update',$permission->id)}}">
            @csrf
            @if($type === 'edit')
            @method('PUT')
            @endif
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div class="flex flex-col">
                <label class="font-semibold text-sm">Permission Name</label>
                <input
